...
class AmenityList(Resource):
    ...
    def post(self):
        ...
        amenity = amenitiesns.facade.create_amenity(data)
    ...
    def get(self):
        amenities = amenitiesns.facade.get_all_amenities()
...
class AmenityResource(Resource):
    ...
    def get(self, amenity_id):
        amenity = amenitiesns.facade.get_amenity(amenity_id)
        if not amenity:
    ...
    def put(self, amenity_id):
        data = request.get_json()
        updated = amenitiesns.facade.update_amenity(amenity_id, data)
        if not updated:
            amenitiesns.abort(404, 'Amenity not found')
